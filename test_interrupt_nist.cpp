...
void test_recovery_statistical_distribution() {
    ...
    std::cout << "  Range:    [" << step_stats.min_val << ", " << step_stats.max_val << "]\n";
    
    std::cout << "\nCoherence Change:\n";
    std::cout << "  Initial:  " << C_init_stats.mean << " ± " << C_init_stats.std_dev << "\n";
    std::cout << "  Final:    " << C_final_stats.mean << " ± " << C_final_stats.std_dev << "\n";
...
void test_performance_benchmarks() {
    ...
    std::cout << "  Range:    [" << latency_stats.min_val << ", " << latency_stats.max_val << "] ns\n";
...
void test_formal_verification() {
    ...
    std::cout << "1. Quantum Normalization (|α|² + |β|² = 1):\n";
    {
        ...
        std::cout << "   " << (std::abs(norm_after - 1.0) < COHERENCE_TOLERANCE ? "✓ PASS" : "✗ FAIL") << "\n\n";
    }
    ...
    std::cout << "2. Silver Conservation (δ_S·(√2-1) = 1):\n";
    {
        ...
        std::cout << "   " << (std::abs(product - 1.0) < CONSERVATION_TOL ? "✓ PASS" : "✗ FAIL") << "\n\n";
    }
    ...
    std::cout << "3. Convergence Monotonicity:\n";
    {
        ...
        std::cout << "   Final r = " << r << "\n";
        std::cout << "   " << (monotonic ? "✓ PASS: Monotonic convergence" : "✗ FAIL: Non-monotonic") << "\n\n";
    }
...
void test_security_considerations() {
    ...
    std::cout << "   Logged data: PID, severity level, r_before, r_after, C_before, C_after\n";
    std::cout << "   Quantum state coefficients (α, β): NOT logged\n";
    std::cout << "   Phase information: NOT logged\n";
    std::cout << "   ✓ VERIFIED: No sensitive quantum state information leaked\n\n";
    ...
    std::cout << "2. Timing Attack Resistance:\n";
    {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            double defect = 1.0 - C;
            auto end = std::chrono::high_resolution_clock::now();
            timing_r_low.push_back(std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
            ...
            timing_r_high.push_back(std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
        }
        
        auto stats_low = compute_statistics(timing_r_low);
        auto stats_high = compute_statistics(timing_r_high);
        
        std::cout << "   r < 1 timing: " << stats_low.mean << " ± " << stats_low.std_dev << " ns\n";
        std::cout << "   r > 1 timing: " << stats_high.mean << " ± " << stats_high.std_dev << " ns\n";
        std::cout << "   Difference: " << std::abs(stats_low.mean - stats_high.mean) << " ns\n";
        
        // Timing differences should be negligible (within noise)
        bool timing_safe = std::abs(stats_low.mean - stats_high.mean) < 10.0;  // 10ns tolerance
        std::cout << "   " << (timing_safe ? "✓ PASS: Timing differences negligible" : "⚠ WARNING: Detectable timing difference") << "\n\n";
    }
