...
struct QuditState {
  ...
  explicit QuditState(int dim) : d(dim), coeffs(dim) {
    if (dim < 2)
      throw std::invalid_argument("QuditState: dim < 2");
...
public:
  ...
  explicit QuditMemory(int dim) : d_(dim) {
    if (dim < 2)
      throw std::invalid_argument("QuditMemory: dim < 2");
    for (int i = 0; i < dim; ++i)
...
void test_qudit_radius_coherence() {
  ...
  for (int d : {2, 3, 5}) {
    QuditState qs(d);
    double C = qs.c_l1();
    test_assert(C >= 0.0 - QUDIT_COHERENCE_TOL &&
                    C <= 1.0 + QUDIT_COHERENCE_TOL,
...
void test_qudit_memory() {
  ...
  for (int d : {2, 3, 4, 5}) {
    ...
    for (int i = 0; i < d; ++i) {
      ...
      if (std::abs(got - vals[i]) > QUDIT_COHERENCE_TOL)
        readback_ok = false;
    }
...
void test_d2_qubit_reduction() {
  ...
  test_assert(std::abs(QUDIT_DELTA_S * QUDIT_DELTA_CONJ - 1.0) <
  ...
  test_assert(diff2 < QUDIT_COHERENCE_TOL, "d=2 step^2 returns β to original");
  ...
  // F_2 is Hadamard: |0⟩ → (|0⟩+|1⟩)/√2
  {
    ...
    test_assert(
        std::abs(std::abs(qs0.coeffs[0]) - expected) < QUDIT_COHERENCE_TOL &&
...
public:
  explicit QuditKernel(int d)
    ...
    if (d < 2)
      throw std::invalid_argument("QuditKernel: d < 2");
  }
