 ...
 * 
 * Formal verification of all theorems from the Pipeline of Coherence derivations.
 * Each theorem is tested with strict assertions at floating point precision.
 */
...
double coherence_sech(double lambda) { 
...
void test_theorem_3() {
    ...
    test_assert(std::abs(sum - 1.0) < TIGHT_TOL, 
                "η² + η² = 1 exactly");
    
    // η = 1/√2 is the unique positive root of 2λ²=1
    double expected_eta = 1.0 / std::sqrt(2.0);
    test_assert(std::abs(ETA - expected_eta) < TIGHT_TOL,
                "η = 1/√2 exact value");
...
void test_section_2() {
    ...
    // |µ| = 1
    test_assert(std::abs(std::abs(MU) - 1.0) < TIGHT_TOL,
                "|µ| = 1 exactly");
    ...
    // This is mathematically guaranteed, we verify by checking all 8 powers are distinct
...
void test_section_3() {
    ...
    test_assert(std::abs(det - 1.0) < TIGHT_TOL,
                "det(R) = 1 exactly");
...
void test_theorem_9() {
    ...
    test_assert(std::abs(alpha_mag - ETA) < TIGHT_TOL,
                "|α| = 1/√2 exactly");
    test_assert(std::abs(beta_mag - ETA) < TIGHT_TOL,
                "|β| = 1/√2 exactly");
    test_assert(std::abs(C - 1.0) < TIGHT_TOL,
                "Forward: |α|=|β|=1/√2 → C=1");
    ...
    test_assert(std::abs(r - 1.0) < FLOAT_TOL && std::abs(C_from_r - 1.0) < FLOAT_TOL,
...
void test_theorem_10() {
    ...
    test_assert(stays_on_circle, "r=1: |ξⁿ| = 1 for all n (stays on unit circle)");
...
void test_theorem_11() {
    ...
    // C(1) = 1 exactly
    test_assert(std::abs(coherence(1.0) - 1.0) < TIGHT_TOL,
                "C(1) = 1 exactly");
    ...
    test_assert(std::abs(dC_dr_at_2) > FLOAT_TOL,
                "dC/dr ≠ 0 at r≠1");
    
    // d²C/dr² < 0 at r=1 confirms maximum
    double d2C_dr2 = (coherence(1.0 + dr) - 2.0 * coherence(1.0) + coherence(1.0 - dr)) / (dr * dr);
    test_assert(d2C_dr2 < -FLOAT_TOL,
                "d²C/dr² < 0 at r=1 (confirms maximum)");
...
void test_prop_4() {
    ...
    test_assert(std::abs(inverse - DELTA_CONJ) < TIGHT_TOL,
                "1/δ_S = √2-1");
