...
\begin{remark}[Continuous-oracle model]
  Theorem~\ref{thm:main-sqrt} is stated in the \emph{continuous-oracle model},
  in which every step has full coherent access to the Dirichlet kernel
  ($r=1$, $\Geff=1$).  Decoherence ($\varepsilon>0$) degrades the advantage
  as analysed in the subsection below.
\end{remark}
...
is $\ge 0.9984$ at all tested scales.
...
\begin{theorem}[Universal scaling limit]\label{thm:universal}
  The maximum sustainable scaling exponent is bounded by
  \begin{equation}
    \alpha_{\max} = 1 + \frac{1}{e} \approx 1.3679,
    \label{eq:alpha-max}
  \end{equation}
  where $1/e \approx 0.3679$ is the universal exponential decay constant.
  \emph{No parameter tuning can exceed this bound.}
\end{theorem}

\noindent
The bound arises from the interplay of three structures:
\begin{enumerate}
...
Main Thm.
  & $\Theta(\sqrt n)$ complexity
  & $T(n)=\Theta(\sqrt n)$, $c\approx 0.19$\\
Thm.\ Jensen
...
\begin{enumerate}
  ...
  \item Noise destroys the $\sqrt{n}$ advantage above $\varepsilon^*$, with
    the universal ceiling $\alpha_{\max}=1+1/e$
    (Theorems~\ref{thm:sharpening}, \ref{thm:universal}).
\end{enumerate}
...
torus, the Ohm--Coherence duality, and the universal ceiling $\alpha_{\max}=1+1/e$
are all encoded in this single point in the second quadrant of the complex plane:
the \textbf{critical complex equilibrium} of the negative real and positive
imaginary eigenvalues, balanced on the unit circle.
