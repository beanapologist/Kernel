/*
 * Qudit Kernel — Extension of Pipeline of Coherence for d-dimensional quantum
 * systems
 *
 ...
 *   QuditOps       : generalized gates — shift X_d, clock Z_d, Fourier F_d,
 * rotation R_d QuditEntangle  : coupling/entanglement for qudits of varying
 * dimensions QuditMemory    : rotational memory addressing in Z/dZ QuditProcess
 * : schedulable unit with a d-dimensional quantum state QuditKernel    : kernel
 * managing qudit processes on d-cycles
 *
 ...
 *   d-cycle step  : c_k *= ω_d^k, ω_d = e^{2πi/d}            (step^d =
 * identity)
 *
...
// Silver constant from Prop 4 (qubit kernel), preserved here for IPC
// compatibility
 ...
 *   Maximum C_d=1 when |c_k|=1/√d for all k (balanced/maximally coherent
 * state). For d=2: C_2 = 2|c_0||c_1| = 2|α||β|  (matches qubit c_l1).
 *
 * Radius r_d:
 *   r_d = √(Σ_{k≥1}|c_k|²/(d-1)) / |c_0|
 *   r_d = 1 ↔ balanced state.  r_d > 1 ↔ excited-dominant.  r_d < 1 ↔
 * ground-dominant. For d=2: r_2 = |c_1|/|c_0| = |β/α|  (matches qubit radius).
 *
 * d-cycle step:
...
struct QuditState {
  ...
  double radius() const {
    if (std::abs(coeffs[0]) < QUDIT_COHERENCE_TOL)
      return 0.0;
    if (d == 2) {
  ...
  // ── d-cycle step ───────────────────────────────────────────────────────────
  // Apply c_k *= ω_d^k.  After d steps returns to original (pure phase
  // rotation).
  void step() {
 ...
 *      c_k^{(1)'} = c_k^{(1)} cos θ + c_k^{(2)} ω_{d1}^k sin θ   (k <
 * min(d1,d2)) c_k^{(2)'} = c_k^{(2)} cos θ - c_k^{(1)} conj(ω_{d2}^k) sin θ θ =
 * coupling_strength * π / (2*max(d1,d2)), bounded to avoid inversion.
 *
...
public:
  ...
  bool phase_couple(QuditState &s1, QuditState &s2) {
    ...
    double theta =
        config_.coupling_strength * M_PI / (2.0 * std::max(s1.d, s2.d));
    ...
    if (config_.preserve_coherence) {
      if (s1.c_l1() > 1.0 + QUDIT_COHERENCE_TOL ||
        ...
        if (config_.log_interactions)
          std::cout
              << "    ✗ QuditEntangle: coherence violation, rolled back\n";
        return false;
      }
    }
   ...
   *     target' = Σ_a |c_a|² · X_d^a |ψ_target⟩  (incoherent mixture
   * approximation) This first-order approximation preserves normalization and
   * gives correct result when control is in a computational basis state. For a
   * full coherent entangling gate, the joint d²-dimensional state would be
   * required.
   */
...
public:
  ...
  void report_stats() const {
    auto s = get_stats();
    std::cout << "  QuditMemory(d=" << s.dimension << "): " << s.total_reads
              << " reads, " << s.total_writes << " writes, " << s.rotation_count
              << " rotations, " << "offset=" << s.rotation_offset << "/"
              << s.dimension << ", " << "capacity=" << s.total_capacity
              << " cells\n";
  }
...
QuditDecoherenceLevel measure_qudit_decoherence(double r) {
  ...
  if (dev <= QUDIT_RADIUS_TOL)
    return QuditDecoherenceLevel::NONE;
  if (dev <= QUDIT_DECOHERENCE_MINOR)
    return QuditDecoherenceLevel::MINOR;
  if (dev <= QUDIT_DECOHERENCE_MAJOR)
...
const char *qudit_decoherence_name(QuditDecoherenceLevel lvl) {
  switch (lvl) {
  case QuditDecoherenceLevel::NONE:
    return "NONE";
  case QuditDecoherenceLevel::MINOR:
    return "MINOR";
  case QuditDecoherenceLevel::MAJOR:
    return "MAJOR";
  case QuditDecoherenceLevel::CRITICAL:
    return "CRITICAL";
  }
  return "";
}
...
struct QuditProcess {
  ...
  void report() const {
    ...
    std::cout << "  PID " << pid << "  d=" << state.d << "  cycle=" << cycle_pos
              << "  r=" << std::setw(10) << r << "  C_ℓ1=" << std::setw(10) << C
              << "  R(r)=" << std::setw(10) << R
...
public:
  explicit QuditKernel(int d)
    ...
    if (std::abs(QUDIT_DELTA_S * QUDIT_DELTA_CONJ - 1.0) >
        QUDIT_CONSERVATION_TOL)
  ...
  void report() const {
    ...
    std::cout << "  Prop 4:  δ_S·(√2-1) = " << QUDIT_DELTA_S * QUDIT_DELTA_CONJ
              << "  (must be 1.0)\n";
...
int main() {
  ...
  for (int d : {2, 3, 4, 5, 8}) {
    QuditState qs(d);
    std::cout << "  d=" << d << "  |ψ_balanced⟩: r=" << qs.radius()
              << "  C_ℓ1=" << qs.c_l1()
  ...
  QuditState qutrit(
      3, {Cx{1.0 / std::sqrt(2.0), 0.0}, Cx{0.5, 0.0}, Cx{0.5, 0.0}});
  std::cout << "    r=" << qutrit.radius() << "  C_ℓ1=" << qutrit.c_l1()
            << "  norm=" << qutrit.norm_sq() << "\n";

  // d=2 recovery: c_0=α=1/√2 (real), c_1=β=(-1+i)/2
  std::cout << "\n  d=2 canonical coherent state (matches qubit kernel):\n";
  QuditState qubit2(2, {Cx{1.0 / std::sqrt(2.0), 0.0}, Cx{-0.5, 0.5}});
  std::cout << "    r=" << qubit2.radius() << "  C_ℓ1=" << qubit2.c_l1()
  ...
  for (int d : {2, 3, 4}) {
    ...
    for (int k = 0; k < d; ++k)
      diff =
          std::max(diff, std::abs(test_state.coeffs[k] - ref_state.coeffs[k]));
  ...
  std::cout << "  QFT on balanced d=4 state:\n";
  {
    ...
    std::cout << "\n  C_ℓ1 after QFT = " << C_after
              << (std::abs(C_after - 1.0) < QUDIT_COHERENCE_TOL
                      ? " ✓ (maximally coherent)"
                      : "")
              << "\n";
  }
...

  {
    ...
    kernel3.spawn("Qutrit-B", [](QuditProcess &p) {
      if (p.cycle_pos == 0) {
        std::cout << "    [Qutrit-B] completed 3-cycle, C_ℓ1=" << p.state.c_l1()
                  << "\n";
      }
    });
...

  {
    ...
    kernel4.spawn("Ququart-Reader", [](QuditProcess &p) {
      if (p.cycle_pos == 2) {
        Cx val = p.mem_read(0);
        std::cout << "    [Reader] read addr[0]=" << val << " at cycle "
                  << p.cycle_pos << "\n";
      }
    });
