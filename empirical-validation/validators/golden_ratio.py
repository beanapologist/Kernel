...
This module validates their algebraic properties against NIST tabulated values:

  1. φ = (1+√5)/2                 (definition)
  2. φ² = φ + 1                   (characteristic polynomial)
  3. φ − 1 = 1/φ                  (reciprocal identity)
  4. φ² − φ − 1 = 0               (minimal polynomial root)
  5. δ_S = 1 + √2                 (silver ratio definition)
  6. δ_S² − 2·δ_S − 1 = 0        (minimal polynomial root for δ_S)
  7. δ_S·(√2 − 1) = 1             (silver conservation: δ_S × (√2−1) = 1)
  8. φ + 1/φ = √5                 (sum identity)
  9. Fibonacci convergence: F(n+1)/F(n) → φ as n → ∞

Both SymPy exact and NumPy numerical checks are included.
"""
...
def validate(data: dict | None = None) -> list[dict[str, Any]]:
    ...
    tol_num = 1e-14   # numerical tolerance
