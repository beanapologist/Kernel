...
constexpr double DECOHERENCE_MINOR =
    0.05; // Minor deviation: RADIUS_TOLERANCE < |r-1| ≤ 0.05
constexpr double DECOHERENCE_MAJOR =
    0.15; // Major deviation: 0.05 < |r-1| ≤ 0.15
...
Vec2 rotate135(Vec2 v) {
  return {-ETA * v.x - ETA * v.y, ETA * v.x - ETA * v.y};
}
...
Regime classify(double r) {
  if (std::abs(r - 1.0) < RADIUS_TOLERANCE)
    return Regime::FINITE_ORBIT;
  return r > 1.0 ? Regime::SPIRAL_OUT : Regime::SPIRAL_IN;
}

const char *regime_name(Regime reg) {
  switch (reg) {
  case Regime::FINITE_ORBIT:
    return "FINITE_ORBIT (r=1, 8-cycle)";
  case Regime::SPIRAL_OUT:
    return "SPIRAL_OUT   (r>1, diverge)";
  case Regime::SPIRAL_IN:
    return "SPIRAL_IN    (r<1, collapse)";
  }
  return "";
}
 ...
 * 3. Triggers handlers: applies corrective actions using coherence function
 * C(r)
 * 4. Recovers coherence: guides process back toward r=1
 *
 * Recovery Strategy:
 * - Measures coherence defect: ΔC = 1 - C(r) where C(r) = 2r/(1+r²) (Theorem
 * 11)
 * - Correction strength proportional to ΔC (coherence-guided recovery)
 * - Severity multipliers scale intervention (0.5/0.8/1.0 for
 * MINOR/MAJOR/CRITICAL)
 * - Preserves normalization |α|²+|β|²=1 and silver conservation δ_S·(√2-1)=1
 * (Prop 4)
...
DecoherenceLevel measure_decoherence(double r) {
  ...
  if (deviation <= RADIUS_TOLERANCE)
    return DecoherenceLevel::NONE;
  if (deviation <= DECOHERENCE_MINOR)
    return DecoherenceLevel::MINOR;
  if (deviation <= DECOHERENCE_MAJOR)
    return DecoherenceLevel::MAJOR;
  return DecoherenceLevel::CRITICAL;
}

const char *decoherence_name(DecoherenceLevel level) {
  switch (level) {
  case DecoherenceLevel::NONE:
    return "NONE";
  case DecoherenceLevel::MINOR:
    return "MINOR";
  case DecoherenceLevel::MAJOR:
    return "MAJOR";
  case DecoherenceLevel::CRITICAL:
    return "CRITICAL";
  }
  return "";
}
 ...
 *    - Severity multiplier scales intervention (0.5 for MINOR, 0.8 for
 * MAJOR, 1.0 for CRITICAL)
...
private:
  ...
  bool apply_recovery(QState &state, DecoherenceLevel level) {
    ...
    if (new_r < MIN_RECOVERY_RADIUS)
      new_r = MIN_RECOVERY_RADIUS;
    if (new_r > MAX_RECOVERY_RADIUS)
      new_r = MAX_RECOVERY_RADIUS;
 ...
 * - Rotational transformations preserve memory coherence across cycle
 * boundaries
 ...
 * - Coherence preservation: addresses maintain relative positions under
 * rotation
 ...
 * - Channel synchronization: messages delivered when receiver at matching
 * position
...
public:
  ...
  struct Config {
    ...
    bool enable_cycle_alignment =
        true;                  // Require cycle position match for delivery
  ...
  std::vector<Message> receive_messages(uint32_t to_pid, uint32_t from_pid,
    ...
    if (config_.enable_coherence_check) {
      if (receiver_coherence < config_.coherence_threshold) {
        if (config_.log_messages) {
          std::cout << "    ✗ IPC: PID " << to_pid << " receive from PID "
                    << from_pid << " BLOCKED (receiver coherence too low: "
                    << receiver_coherence << ")\n";
        }
        return delivered;
      }
    ...
    while (it != channel.queue.end()) {
      ...
      if (config_.log_messages) {
        std::cout << "    ✉ IPC: PID " << to_pid << " ← PID " << from_pid
...
private:
  ...
  std::vector<Channel>::iterator find_channel(uint32_t from_pid,
                                              uint32_t to_pid) {
...
struct Process {
  ...
  bool corollary13() const {
    ...
    bool palindrome_exact =
        (std::abs(state.palindrome()) < COHERENCE_TOLERANCE);
...
public:
  ...
  bool interact(Process &p1, Process &p2) {
    ...
    if (config_.log_interactions) {
      std::cout << "    ⊗ Interaction: PID " << p1.pid << " ↔ PID " << p2.pid
...
public:
  ...
  void tick() {
    ...
    // Note: Memory addressing rotation could be synchronized with cycle
    // progression Currently, rotation is performed explicitly via
    // memory().rotate_addressing() to give precise control over when memory
    // maps are transformed.
  ...
  void report() const {
    ...
    std::cout << "\n  Prop 4:  δ_S·(√2-1) = " << DELTA_S * DELTA_CONJ
              << "  (must be 1.0)\n";
    ...
    if (!processes_.empty()) {
      ...
      std::cout << "  Thm 14:  C(r)=" << C << "  sech(λ)=" << sc << "  match="
                << (std::abs(C - sc) < COHERENCE_TOLERANCE ? "✓" : "✗") << "\n";
    }
...
int main() {
  ...
  for (double r : {0.5, 0.9, 1.0, 1.1, 2.0}) {
    ...
    std::cout << std::fixed << std::setprecision(6) << "  r=" << r
              << "  C=" << C << "  R=" << R
              << "  sech(λ)=" << coherence_sech(lm) << "  " << regime_name(reg)
              << "\n";
  }
  ...
  for (uint32_t i = 0; i < 16; i += 2) {
    ...
    Cx value =
        Cx{scale * std::cos(i * M_PI / 8), scale * std::sin(i * M_PI / 8)};
  ...
  for (uint32_t i = 0; i < 16; i += 2) {
    ...
    std::cout << "   addr[" << std::setw(2) << i << "] = " << std::setw(10)
              << value.real() << " + " << std::setw(10) << value.imag()
              << "i\n";
  }
  ...
  for (uint32_t lin_addr : {0, 1, 7, 8, 15, 16, 23}) {
    ...
    std::cout << "   Linear[" << std::setw(2) << lin_addr << "] → Bank["
              << (int)addr.bank << "], Offset[" << addr.offset << "]\n";
  }
  ...
  mem_kernel.spawn("MemoryReader", [](Process &p) {
    ...
    if (p.cycle_pos == 6) {
      ...
      std::cout << "    [MemoryReader] read addr[0] = " << value.real() << " + "
                << value.imag() << "i at cycle " << (int)p.cycle_pos << "\n";
    }
  });
  ...
  for (size_t i = 0; i < banks.size(); ++i) {
    std::cout << "   Bank[" << i << "]: " << banks[i].data.size() << " cells, "
  ...
  std::cout << "Decoherence interrupts will trigger when |r-1| exceeds "
               "thresholds\n\n";
  int_kernel.run(8);
  ...
  for (double rate : {0.3, 0.6, 0.9}) {
    ...
    std::cout << "Recovery rate = " << std::setprecision(1) << std::fixed
              << rate << ":\n";
    test_kernel.report();
  }
  ...
  std::cout << "✓ Recovery handlers using coherence function C(r) for "
               "correction strength\n";
  ...
  std::cout
      << "   Demonstrating coherence checks during message passing...\n\n";
  ...
  std::cout
      << "   Multiple processes exchanging quantum state information...\n\n";
  ...
  cycle_kernel.spawn("CycleReceiver", [](Process &p) {
    ...
    if (p.cycle_pos == 0) {
      auto msgs = p.receive_from(1);
      if (msgs.empty()) {
        std::cout << "    [CycleReceiver] No messages at position 0 (cycle "
                     "alignment enforced)\n";
      }
    }
  });
