...
// ── StepResult
// ──────────────────────────────────────────────────────────────── Return value
// of LadderChiralSearch::ladder_step.
//
...
// ── LadderChiralSearch
// ──────────────────────────────────────────────────────── Chiral ladder search
// over n candidate states.
//
// Members
...
struct LadderChiralSearch {
  ...
  //   4. Stores β amplitudes in an Eigen::VectorXcd for efficient norm
  //   computation.
  ...
  StepResult ladder_step(size_t n) {
    ...
    // r_i = |β_i| / |α_i|; α is unchanged by chiral_nonlinear, so |α| =
    // CHIRAL_ETA. Averaging across states reveals how the kick redistributes
    // coherence between the marked target and the unmarked background.
    double coh_sum = 0.0;
