...
### 6.4  Simulation: circular coherence R(t)

![Reflection vs EMA circular coherence](reflection_demo.png)

The figure above (`reflection_demo.png`, generated by `plot_reflection_effect()`
in `kernelsync_energy_proxy_sim.py`) shows $R(t)$ over 40 iterations for
$N = 20$ nodes initialised with uniformly random phases:

- **Exact Grover ($g=1$, blue)** — $R$ is **conserved** at the initial value
  $R_0$ for all iterations.  The individual phases undergo a period-2 orbit
  (each application maps $\hat\psi_j \to 2\bar\psi - \hat\psi_j$ then back),
  but the isometry property $|e^{2i\bar\psi}| = 1$ keeps $R$ exactly constant.
- **KernelSync EMA ($g=0.3$, orange)** — $R$ is **monotonically non-decreasing**,
  growing toward $1$ as the phases contract toward their mean.  After $k$ steps
  the deviation norm shrinks as $(1-g)^k$, driving $R \to 1$ exponentially.

This directly illustrates the conserved-quantity analysis in §6.3: the EMA
loop sacrifices norm-conservation (the hallmark of exact Grover unitarity) in
exchange for guaranteed exponential convergence.
