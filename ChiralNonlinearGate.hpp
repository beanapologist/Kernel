 ...
 *   Im(β) ≤ 0 domain: β' = µ·β              (linear — classical gates,
 * reversible) Im(β) > 0 domain: β' = µ·β + k·(µ·β)·|µ·β|  (non-linear —
 * reversibility shattered)
 *
 * Integration:
...
// µ = e^{i3π/4}: exact 135° balance rotation (mirrors MU in
...
// ──────────────────────────────────────────────────── 8-cycle validation:
// observe linear behaviour on Im ≤ 0 steps and quadratic magnitude growth on Im
// > 0 steps.
