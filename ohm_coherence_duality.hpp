...
// ── metallic_oscillating_phases: constructive interference / phase lensing ───
//
// Models the "metallic medium" focusing effect: phases are projected toward an
// alignment angle θ_align with an amplitude proportional to the current
// circular coherence R of the input ensemble.  At R = 1 (perfect coherence)
// the phases already sit at the focal point and the output is undisturbed.
// At intermediate R the lensing amplitude equals R · α, so higher coherence
// creates stronger focusing — a positive feedback loop.
//
// Output rule:
//   ψ_j^out = ψ_j − R·α · wrap(ψ_j − θ_align)
//
// Parameters:
//   phases      — input phases ψ̂_j (radians)
//   align_angle — focal alignment angle θ_align (radians)
//   alpha       — coherence amplification factor α ≥ 0 (default 1.0)
//
...
metallic_oscillating_phases(const std::vector<double> &phases,
  ...
  if (N == 0)
    return {};
  // Compute circular coherence R of the input ensemble
  ...
  double amp = R * alpha; // lensing amplitude = coherence × gain

  // Project each phase toward the alignment angle
  std::vector<double> out(N);
  for (int j = 0; j < N; ++j) {
    double delta = phases[j] - align_angle;
    // wrap to (−π, π]
    delta = delta - 2.0 * OHM_PI * std::floor((delta + OHM_PI) / (2.0 * OHM_PI));
    out[j] = phases[j] - amp * delta;
  }
  return out;
}

// ── interaction_energy: lensing focal intensity at coherence R ────────────────
//
// Computes the constructive-interference energy generated by the phase medium.
// Analogous to optical lensing intensity at the focal point, where power scales
// as the square of the coherence amplitude:
//
//   E_interact(R, N, g) = R² · N · g
//
// At R = 0 (fully incoherent): E = 0   — no constructive interference.
// At R = 1 (perfectly coherent): E = N·g — maximum focal energy.
// The R² factor mirrors the Born rule: probability/power ∝ amplitude².
//
